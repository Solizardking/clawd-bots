...
export function createCoordinatorInferenceRouter(options: {
  ...
  const execute = async (provider: Exclude<SandInferenceProvider, "cursor">, args: Record<string, unknown>) => {
    ...
    const withLocalTools = async (): Promise<Record<string, any>[]> => {
      ...
      return [...(Array.isArray(remote) ? remote as Record<string, any>[] : []), ...BUNDLED_SKILL_ROUTED_TOOLS, ...pumpRoutedTools(), ...kernelTools, ...e2bRoutedTools(), ...browserUseRoutedTools(), ...grokMediaRoutedTools(), ...solanaTradingRoutedTools(), ...solanaAgentRoutedTools(), ...clawdGatewayRoutedTools(), ...cloudBoxRoutedTools(), ...webRoutedTools()];
    };
