...
import { isSandInferenceProvider, normalizeSandOpenRouterModel } from "../shared/inference-router.js";
import { normalizeSandAgentCustomization } from "../shared/agent-customization.js";
...
import { synthesizeGrokSpeech } from "../node-agent-coordinator/grok-media-tools.js";
...
export interface MainEdgeDeps {
  ...
  readonly solana: UnknownRecord;
  readonly telegram: UnknownRecord;
...
export function createMainEdgeHandlers(deps: MainEdgeDeps): HandlerMap {
  const handlers: HandlerMap = {
    ...
    getInferenceRouter: async () => { const settings = await deps.readHostSettingsFromBox().catch(() => ({} as UnknownRecord)); const provider = invoke(deps.settingsStore, "getInferenceProvider"); return { provider: isSandInferenceProvider(provider) ? provider : "openrouter", model: invoke(deps.settingsStore, "getInferenceRouterModel"), usage: settings.inferenceRouterUsage ?? invoke(deps.settingsStore, "getInferenceRouterUsage") ?? null, local: getLocalInferenceCliStatus(), cache: { enabled: invoke(deps.settingsStore, "getInferenceRouterCacheEnabled"), ttlSeconds: invoke(deps.settingsStore, "getInferenceRouterCacheTtlSeconds") } }; },
    setInferenceRouter: async (raw) => { const request = req(raw); const provider = request.provider; invariant(isSandInferenceProvider(provider), "Unknown inference provider."); const model = normalizeSandOpenRouterModel(request.model) ?? invoke(deps.settingsStore, "getInferenceRouterModel"); invariant(typeof model === "string" && model.length > 0, "Unknown OpenRouter model id."); invoke(deps.settingsStore, "setInferenceProvider", provider); if (request.model != null) invoke(deps.settingsStore, "setInferenceRouterModel", model); const cacheRequest = typeof request.cache === "object" && request.cache != null ? request.cache as { enabled?: unknown; ttlSeconds?: unknown } : null; if (cacheRequest != null && typeof cacheRequest.enabled === "boolean") invoke(deps.settingsStore, "setInferenceRouterCacheEnabled", cacheRequest.enabled); if (cacheRequest != null && typeof cacheRequest.ttlSeconds === "number") invoke(deps.settingsStore, "setInferenceRouterCacheTtlSeconds", cacheRequest.ttlSeconds); const settings = await deps.syncHostSettingsToBox({ inferenceProvider: provider, inferenceRouterModel: model }).catch(() => null); return { provider, model, usage: settings?.inferenceRouterUsage ?? invoke(deps.settingsStore, "getInferenceRouterUsage") ?? null, local: getLocalInferenceCliStatus(), cache: { enabled: invoke(deps.settingsStore, "getInferenceRouterCacheEnabled"), ttlSeconds: invoke(deps.settingsStore, "getInferenceRouterCacheTtlSeconds") } }; },
    ...
    getAgentCustomization: () => invoke(deps.settingsStore, "getAgentCustomization") ?? null,
    setAgentCustomization: (raw) => { const value = normalizeSandAgentCustomization(req(raw).customization); invoke(deps.settingsStore, "setAgentCustomization", value); return invoke(deps.settingsStore, "getAgentCustomization") ?? null; },
    ...
    speechSynthesize: async (raw) => { const request = req(raw); const text = typeof request.text === "string" ? request.text.trim() : ""; invariant(text.length > 0 && text.length <= 4096, "speechSynthesize requires 1-4096 characters of text."); const voice = typeof request.voice === "string" && request.voice.trim().length > 0 ? request.voice.trim() : undefined; return await Promise.resolve(synthesizeGrokSpeech({ text, ...(voice === undefined ? {} : { voice }) })); },
    ...
    solanaGetStatus: () => invoke(deps.solana, "getStatus"),
    solanaCreateWallet: (raw) => invoke(deps.solana, "createWallet", req(raw)),
    solanaListWallets: () => invoke(deps.solana, "listWallets"),
    solanaGetWalletAssets: (raw) => invoke(deps.solana, "getWalletAssets", req(raw)),
    solanaGetAsset: (raw) => invoke(deps.solana, "getAsset", req(raw)),
    solanaSearchAssets: (raw) => invoke(deps.solana, "searchAssets", req(raw)),
    solanaGenerateLocalWallet: (raw) => invoke(deps.solana, "generateLocalWallet", req(raw)),
    solanaListLocalWallets: () => invoke(deps.solana, "listLocalWallets"),
    solanaRevealLocalWalletSecret: (raw) => invoke(deps.solana, "revealLocalWalletSecret", req(raw)),

    telegramGetStatus: () => invoke(deps.telegram, "getStatus"),
    telegramSaveToken: (raw) => invoke(deps.telegram, "saveToken", req(raw)),
    telegramClearToken: () => invoke(deps.telegram, "clearToken"),
    telegramStart: () => invoke(deps.telegram, "start"),
    telegramStop: () => invoke(deps.telegram, "stop"),
  };
