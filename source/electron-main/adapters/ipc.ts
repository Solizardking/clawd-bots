...
export function createProductionSecretsIpcRegistrar(): ProductionIpcRegistrar {
  return (context, ipc) => {
    const guards = createTrustedSenderGuards(context.getTrustedContents);
    ipc.handle("sand:ows-create", (event, request) => { guards.assertTrustedSecretsSender(event); return context.solana.createOwsWallet(request); });
    ipc.handle("sand:ows-list", (event) => { guards.assertTrustedSecretsSender(event); return context.solana.listOwsWallets(); });
    ipc.handle("sand:ows-requests", (event) => { guards.assertTrustedSecretsSender(event); return context.solana.getOwsRequests(); });
    ipc.handle("sand:ows-cancel", (event, request) => { guards.assertTrustedSecretsSender(event); return context.solana.cancelOwsRequest(request); });
    registerSecretsIpc({
      ipcMain: ipc,
