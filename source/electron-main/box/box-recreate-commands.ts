...
export function createSandRecreateCommands<TArgs>(deps: { readonly connector: { recreate?: (args: TArgs) => Promise<RecreateResult>; forceRecreate?: () => Promise<RecreateResult> }; readonly noteRecreateAccepted: (operationId: RecreateOperationId | null) => void }) {
  ...
  return {
    ...
    async forceRecreateComputer(): Promise<RecreateResult> { if (deps.connector.forceRecreate == null) return { status: "rejected", reason: "Reset Clawd Bot's Computer is unavailable without a backend connection." }; const result = await deps.connector.forceRecreate(); noteIfStarted(result); return result; },
  };
}
