...
import { renderBundledSkillsCatalog } from "../../../shared/bundled-trading-skills.js";
import { composeSandAgentCustomizationPromptBlock, type SandAgentCustomization } from "../../../shared/agent-customization.js";
import { createOpenRouterInstrumentedFetch, SAND_OPENROUTER_CACHE_DEFAULT_TTL_SECONDS, type SandOpenRouterLastRoute } from "../../../shared/open-router-cache.js";
...
function xaiCredential(): string {
  const hosted = hostedProviderConfig({ ...persistedSecrets(), ...process.env });
  if (hosted) return hosted.token;
  const value = process.env.XAI_API_KEY?.trim() || persistedSecrets().XAI_API_KEY?.trim();
  if (value == null || value.length === 0) throw new Error("xAI needs XAI_API_KEY. Add it in Settings → Router.");
  return value;
}

function configuredOpenRouterModel(): string {
  return configuredOpenRouterModels()[0]!;
}

function configuredOpenRouterModels(): string[] {
  let stored: string | undefined;
  try {
    stored = new SandSettingsStore(join(getSandRootDir(), "settings.json")).load().inferenceRouterModel;
  } catch {}
  return resolveOpenRouterModelChain(process.env, stored);
}

function configuredXaiModel(): string {
  const selected = process.env.SAND_XAI_MODEL?.trim();
  if (selected != null && selected.length > 0) return selected;
  try {
    const stored = new SandSettingsStore(join(getSandRootDir(), "settings.json")).load().inferenceRouterModel;
    // The shared router-model field may hold another provider's id (e.g. an
    // OpenRouter slug); only reuse it when it names a Grok model.
    if (stored != null && /^grok[-_.a-z0-9]*$/i.test(stored)) return stored;
  } catch {}
  return SAND_DEFAULT_XAI_MODEL;
}

function providerPrompt(messages: readonly ProviderMessage[], systemPrompt: string = GROK_ROUTER_SYSTEM_PROMPT): string {
  const rendered = messages.map(message => {
    const content = typeof message.content === "string" ? message.content : JSON.stringify(message.content);
    return `${message.role.toUpperCase()}: ${content}`;
  }).join("\n\n");
  return `${systemPrompt}\n\nContinue this Clawd Bot conversation.\n\n${rendered}`;
}

export function routedSystemPrompt(customization?: SandAgentCustomization | null): string {
  const bundled = renderBundledSkillsCatalog();
  return `${GROK_ROUTER_SYSTEM_PROMPT}${composeSandAgentCustomizationPromptBlock(customization)}\n\n${PAYBOX_INSTRUCTIONS}${bundled.length === 0 ? "" : `\n\n${bundled}`}`;
}
...
function codexCredentials(): CodexCredentials {
  ...
  if (parsed?.auth_mode !== "chatgpt" || typeof accessToken !== "string" || accessToken.length === 0 || typeof refreshToken !== "string" || refreshToken.length === 0 || typeof idToken !== "string" || idToken.length === 0 || typeof accountId !== "string" || accountId.length === 0) {
    throw new Error("Codex is not signed in with ChatGPT. Run `codex login`, then reopen Clawd Bot.");
  }
...
function codexExecutor(messages: readonly ProviderMessage[], invocationId: string, definitions?: readonly Loose[], executeTool?: RoutedToolExecutor, onUsage?: (usage: UsageRecord) => void, systemPrompt: string = GROK_ROUTER_SYSTEM_PROMPT) {
...
function claudeExecutor(messages: readonly ProviderMessage[], invocationId: string, onUsage?: (usage: UsageRecord) => void, mcpServerUrl?: string, systemPrompt: string = GROK_ROUTER_SYSTEM_PROMPT) {
  const executable = resolveClaudeCodeCliPath();
  if (executable == null) throw new Error("Claude Code is not installed. Install and sign in to Claude Code, then reopen Clawd Bot.");
  const usage = deferred<{ promptTokens: number; completionTokens: number; totalTokens: number }>();
  const extendedUsage = deferred<{ inputTokens: number; outputTokens: number; cacheReadTokens: number; cacheWriteTokens: number; maxTokens: number }>();
  const resultResponse = deferred<ReturnType<typeof response>>();
  const metadata = deferred<Record<string, unknown>>();
...
function streamRoutedText(model: LanguageModelV1, messages: readonly ProviderMessage[], invocationId: string, definitions?: readonly Loose[], executeTool?: RoutedToolExecutor, onUsage?: (usage: UsageRecord) => void, systemPrompt: string = GROK_ROUTER_SYSTEM_PROMPT) {
  const tools = toToolSet(definitions, executeTool);
  const result = streamText({ model, system: systemPrompt, messages: messages as CoreMessage[], ...(tools === undefined ? {} : { tools }), toolCallStreaming: true, maxSteps: tools === undefined ? 1 : 8 });
...
function openRouterCacheOptions(): { cacheEnabled: boolean; cacheTtlSeconds: number } {
  try {
    const settings = new SandSettingsStore(join(getSandRootDir(), "settings.json")).load();
    return { cacheEnabled: settings.inferenceRouterCacheEnabled === true, cacheTtlSeconds: settings.inferenceRouterCacheTtlSeconds ?? SAND_OPENROUTER_CACHE_DEFAULT_TTL_SECONDS };
  } catch { return { cacheEnabled: false, cacheTtlSeconds: SAND_OPENROUTER_CACHE_DEFAULT_TTL_SECONDS }; }
}

function recordOpenRouterRoute(route: SandOpenRouterLastRoute): void {
  try { new SandSettingsStore(join(getSandRootDir(), "settings.json")).recordOpenRouterLastRoute(route); } catch {}
}
...
function openRouterAttributionHeaders(): Record<string, string> {
  const referer = process.env.SAND_OPENROUTER_REFERER?.trim() || "https://solanaclawd.com";
  const title = process.env.SAND_OPENROUTER_TITLE?.trim() || "SolanaClawd";
  return {
    "HTTP-Referer": referer,
    "X-OpenRouter-Title": title,
    ...(process.env.SAND_OPENROUTER_TITLE?.trim() ? {} : { "X-Title": title }),
    "X-OpenRouter-Categories": process.env.SAND_OPENROUTER_CATEGORIES?.trim() || "personal-agent,general-chat",
  };
}

function openRouterExecutor(messages: readonly ProviderMessage[], invocationId: string, definitions?: readonly Loose[], executeTool?: RoutedToolExecutor, onUsage?: (usage: UsageRecord) => void, systemPrompt?: string) {
...
function xaiExecutor(messages: readonly ProviderMessage[], invocationId: string, definitions?: readonly Loose[], executeTool?: RoutedToolExecutor, onUsage?: (usage: UsageRecord) => void, systemPrompt?: string) {
  const id = configuredXaiModel();
  const model: LanguageModelV1 = createOpenAI({ apiKey: xaiCredential(), baseURL: "https://api.x.ai/v1", compatibility: "compatible", name: "xai", fetch: createHostedProviderFetch(hostedProviderConfig({ ...persistedSecrets(), ...process.env })) }).chat(id as any);
  return streamRoutedText(model, messages, invocationId, definitions, executeTool, onUsage, systemPrompt);
}
...
class ProviderPromptExecutor extends BasePromptExecutor<ProviderMessage> {
  ...
  stream(_ctx: unknown, invocationId = crypto.randomUUID(), definitions?: readonly Loose[]) {
    ...
    if (this.provider === "xai") return xaiExecutor(this.getMessages(), invocationId, definitions, undefined, this.onUsage);
...
export function createProviderPromptSession(provider: RoutedProvider): { getModelId(): string; getExecutor(state?: unknown): PromptExecutor } {
  const modelId = provider === "codex" ? configuredCodexModel() : provider === "claude-code" ? "claude-code" : provider === "xai" ? configuredXaiModel() : configuredOpenRouterModel();
...
export async function runRoutedProviderText(provider: RoutedProvider, messages: readonly ProviderMessage[], options?: {
  ...
  readonly agentCustomization?: SandAgentCustomization | null;
}): Promise<string> {
  const invocationId = crypto.randomUUID();
  const onUsage = (usage: UsageRecord) => recordRoutedUsage(provider, usage);
  const systemPrompt = routedSystemPrompt(options?.agentCustomization);
  const result = provider === "codex"
    ? codexExecutor(messages, invocationId, options?.tools, options?.executeTool, onUsage, systemPrompt)
    : provider === "claude-code"
      ? claudeExecutor(messages, invocationId, onUsage, options?.mcpServerUrl, systemPrompt)
      : provider === "xai"
        ? xaiExecutor(messages, invocationId, options?.tools, options?.executeTool, onUsage, systemPrompt)
        : openRouterExecutor(messages, invocationId, options?.tools, options?.executeTool, onUsage, systemPrompt);
  let text = "";
