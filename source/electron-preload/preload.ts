...
export function createDesktopPreloadBridge(options: {
  ...
  const desktop: Record<string, any> = {
    ...
    ows: {
      createWallet: (request: {name: string; password: string; requestId?: string}) => ipc.invoke("sand:ows-create", request),
      listWallets: () => ipc.invoke("sand:ows-list"),
      requests: () => ipc.invoke("sand:ows-requests"),
      cancel: (id: string) => ipc.invoke("sand:ows-cancel", {id}),
    },
    solana: {
      getStatus: () => edge("solanaGetStatus"),
      createWallet: (name: string) => edge("solanaCreateWallet", { name }),
      listWallets: () => edge("solanaListWallets"),
      getWalletAssets: (request: { ownerAddress: string; page?: number; limit?: number; showFungible?: boolean; showNativeBalance?: boolean }) => edge("solanaGetWalletAssets", request),
      getAsset: (id: string) => edge("solanaGetAsset", { id }),
      searchAssets: (request: { ownerAddress: string; tokenType?: string; limit?: number }) => edge("solanaSearchAssets", request),
      generateLocalWallet: (name: string) => edge("solanaGenerateLocalWallet", { name }),
      listLocalWallets: () => edge("solanaListLocalWallets"),
      revealLocalWalletSecret: (name: string) => edge("solanaRevealLocalWalletSecret", { name }),
    },
    telegram: {
      getStatus: () => edge("telegramGetStatus"),
      saveToken: (token: string) => edge("telegramSaveToken", { token }),
      clearToken: () => edge("telegramClearToken"),
      start: () => edge("telegramStart"),
      stop: () => edge("telegramStop"),
    },
    speech: {
      synthesize: (request: { text: string; voice?: string }) => edge("speechSynthesize", request),
    },
    agent: {
      ...
      setInferenceRouter: (provider: string, model?: string, cache?: { enabled?: boolean; ttlSeconds?: number }) => edge("setInferenceRouter", { provider, ...(model == null ? {} : { model }), ...(cache == null ? {} : { cache }) }),
      getBoxRuntime: () => edge("getBoxRuntime"),
      setBoxRuntime: (mode: string) => edge("setBoxRuntime", { mode }),
      getAgentCustomization: () => edge("getAgentCustomization"),
      setAgentCustomization: (customization: unknown) => edge("setAgentCustomization", { customization }),
      clientPersistence: {
