...
//    EmbeddedCuaDriverHost so TCC grants attribute to Clawd Bot and the
//    driver inherits them. One prompt, named Clawd Bot, out of the box.
...
const HOST_BUNDLE_ID = "com.clawdbot.app";
...
async function attachStandalone() {
  ...
  if (!(await socketAlive(STANDALONE_SOCKET))) {
    ...
    // machine already granted — instead of the freshly signed Clawd Bot.
...
async function startEmbedded(binary) {
  ...
  // Clawd Bot rather than to a terminal or helper process.
  ...
  if (!sdk.hasRequiredMacOSPermissions(permissionStatus)) {
    ...
    throw new Error(`${missing || "macOS permissions"} required; grant access in System Settings and restart Clawd Bot`);
  }
