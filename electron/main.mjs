...
if (process.platform === "linux") {
  ...
  app.setDesktopName("com.clawdbot.app.desktop");
}
...
if (!app.requestSingleInstanceLock()) {
  console.log("[desktop] Clawd Bot is already running — focusing that window");
  process.exit(0);
}
...
async function secureComposioConfig() {
  const dataDir = process.env.OMB_DATA_DIR || path.join(app.getPath("home"), ".clawdbot");
...
// goes to server.log in the OS log dir (~/Library/Logs/Clawd Bot on macOS,
// Console.app-visible; %APPDATA%\Clawd Bot\logs on Windows), which is also
...
async function startServerOn(port) {
  if (!credentialStoreUnavailable && typeof secureCredentials.solanaSealKey !== "string") {
    secureCredentials.solanaSealKey = randomBytes(32).toString("base64");
    try {
      await saveSecureCredentials(secureCredentials);
    } catch {
      /* file-mode seal in the server data dir still works */
    }
  }
  ...
  const childEnv = managedComposioChildEnvironment(composioBrokerUrl(), secureCredentials, {
    ...
    ...(typeof secureCredentials.solanaSealKey === "string" && secureCredentials.solanaSealKey
      ? { OMB_SOLANA_SEAL_KEY: secureCredentials.solanaSealKey }
      : {}),
  });
...
function syncManagedComposioCredentials() {
  ...
  try {
    serverProc.postMessage({
      type: "clawdbot:managed-composio",
...
const ERROR_PAGE =
  ...
  encodeURIComponent(
    `<body style="margin:0;display:flex;align-items:center;justify-content:center;height:100vh;background:#070707;color:#fcfcfc;font:15px -apple-system,system-ui"><div style="text-align:center;max-width:360px"><div style="font-size:40px">🐭</div><h2 style="font-weight:600;margin:12px 0 6px">Couldn't start the bot server</h2><p style="color:#fcfcfc99;line-height:1.5">Something else is using its ports. Quit and reopen Clawd Bot — if it keeps happening, restart your computer.</p></div></body>`,
  );

const DEV_UNAVAILABLE_PAGE =
  "data:text/html;charset=utf-8," +
  encodeURIComponent(
    `<body style="margin:0;display:flex;align-items:center;justify-content:center;height:100vh;background:#070707;color:#fcfcfc;font:15px -apple-system,system-ui"><div style="text-align:center;max-width:420px"><div style="font-size:40px">🐭</div><h2 style="font-weight:600;margin:12px 0 6px">Waiting for the app UI</h2><p style="color:#fcfcfc99;line-height:1.5">The desktop shell loads <code style="color:#fcfcfc">http://127.0.0.1:5199</code>. Start it with <code style="color:#fcfcfc">pnpm dev</code> (and <code style="color:#fcfcfc">npm run clawd:server</code>) in the repo, then reopen this window.</p></div></body>`,
  );
...
function openDesktopViewer(owner, rawUrl, rawTitle, contextId) {
  if (!owner || owner.isDestroyed()) throw new Error("The Clawd Bot window is unavailable");
  ...
  const viewer = new BrowserWindow({
    ...
    webPreferences: {
      ...
      partition: "clawdbot-desktop-viewer",
    },
  });
...
function createWindow() {
    ...
    ...(isMac
      ...
      : process.platform === "win32"
        ? {
            ...
            titleBarOverlay: { color: "#0b0714", symbolColor: "#14F195", height: 60 },
          }
        : {}),
    webPreferences: {
      contextIsolation: true,
  ...
  if (app.isPackaged) {
    ...
    // Unpackaged `electron .` paints backgroundColor until Vite answers.
    // Without a retry the failed load stays a blank black window.
    let tries = 0;
    let retrying = false;
    const tryDevUi = () => {
    ...
    win.webContents.on("did-fail-load", (_event, _code, _description, failedUrl, isMainFrame) => {
      ...
      if (!String(failedUrl || "").startsWith(DEV_URL)) return;
      tries += 1;
      if (tries > 80) {
        void win.loadURL(DEV_UNAVAILABLE_PAGE);
        return;
      }
      if (retrying) return;
      retrying = true;
...
// Bots hand users files as markdown links to paths inside the Clawd Bot
...
// renderer-controlled, so it must resolve inside ~/.clawdbot and be a
...
// live desktop inside Clawd Bot instead of sending the person to a browser.
...
const CREDENTIAL_PATCH = {
  ...
  heliusApiKey: (value) => ({ solana: { heliusApiKey: value } }),
  phantomOrganizationId: (value) => ({ solana: { phantomOrganizationId: value } }),
  phantomAppId: (value) => ({ solana: { phantomAppId: value } }),
  phantomApiPrivateKey: (value) => ({ solana: { phantomApiPrivateKey: value } }),
  jupiterApiKey: (value) => ({ solana: { jupiterApiKey: value } }),
};
...
app.whenReady().then(async () => {
  if (app.isPackaged) app.setAsDefaultProtocolClient("clawdbot");
