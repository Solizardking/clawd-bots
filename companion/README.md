...
Clawd Bot's harness listens on `127.0.0.1` and nothing else, which is the
...
With the harness already up (`npm run clawd:server`), from the repo root:
...
harness is running on its own — a headless box, or `npm run clawd:server` in a
...
| `OMB_COMPANION_DIR` | `~/.clawdbot-companion` | paired devices live here |
...
back to `Clawd Bot` when the harness is not up or has no profile. Read once
