  ...
  return function handle(req: IncomingMessage, res: ServerResponse): void {
    ...
    if (isCloudDesktopJoin(method, path) && !device?.cloudDesktopAccess) {
      return sendJson(res, 403, {
        error: "cloud desktop access is off for this phone — enable it in Clawd Bot → Settings → Phone",
      });
    }
      ...
      (harness) => {
        ...
        if (method === "GET" && path === "/api/health") {
          ...
          const fail = () => {
            if (finished) return;
            finished = true;
            sendJson(res, 502, { error: "Clawd Bot is not ready on this computer" });
          };
          ...
          harness.on("end", () => {
            ...
            if (
              ...
              (identity as { app?: unknown } | null)?.app !== "clawdbot"
            ) {
              fail();
              return;
            }
            finished = true;
            sendJson(res, 200, { app: "clawdbot" });
          });
          return;
        }
        ...
        if (isStream) {
          ...
          if (tracksDeviceConnection && currentDevice?.id !== device?.id) {
            ...
            return sendJson(res, 401, {
              error: "pair this device from Phone settings in Clawd Bot on your computer",
            });
          }
          const disconnect = () => {
        ...
        harness.on("data", (chunk: Buffer) => {
          ...
          if (size > MAX_JSON_BODY_BYTES) {
            ...
            else sendJson(res, 502, { error: "the response from Clawd Bot was too large" });
            return;
          }
          chunks.push(chunk);
        });
    ...
    upstream.on("error", () => {
      ...
      sendJson(
        ...
        timedOut ? 504 : 502,
        timedOut
          ? { error: "Clawd Bot did not respond" }
          : { error: "Clawd Bot is not running on this computer" },
      );
    });
    req.pipe(upstream);
  };
