 ...
 * record here:
 *
 * - **Shared records (§10.2).** The PTR of `_clawdbot._tcp.local` is
...
export interface ServiceInfo {
  ...
  type: string;
  port: number;
  /** the name our A records claim, e.g. "clawdbot-1a2b3c4d.local" */
  host: string;
  addresses: string[];
...
export function dnsLabel(text: string, fallback = "Clawd Bot"): string {
  let label = text
    .replace(/[\u0000-\u001f\u007f]/g, "")
    .replace(/\./g, " ")
    .trim();
