...
/** 16px clawd + name, shown once per sender cluster. */
...
export function GroupView({ group }: { group: Group }) {
  ...
  // Static clawds: one per member, a ring + dot on whoever is working.
  const memberClawds = members.map((b) => (
    <span
      key={b.id}
    ...
    >
      <ClawdAvatar color={b.color} state={normalizeState(b.mascotExpression) ?? "happy"} size={24} animated={false} />
  ...
  return (
    <main className="relative flex h-full min-w-0 flex-1 flex-col bg-app">
      ...
      {/* Header: static member clawds; a ring + dot marks the working bot. */}
      <div
        className={cn(
