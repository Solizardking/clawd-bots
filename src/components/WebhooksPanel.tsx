...
function suggestedName(prompt: string, bot?: Bot) {
  ...
  return first || `${bot?.name ?? "Clawd"} webhook`;
}
...
function WebhookEditor({ webhook, bots, onClose, onCredential }: { webhook?: WebhookTrigger; bots: Bot[]; onClose: () => void; onCredential: (credential: WebhookCredential, webhookId: string) => void }) {
  ...
  const [runOn, setRunOn] = useState<RoutineRunOn>(webhook?.runOn ?? "clawd");
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 p-5 backdrop-blur-sm" onMouseDown={(event) => event.target === event.currentTarget && onClose()}>
      <div className="flex max-h-[90vh] w-full max-w-[590px] flex-col overflow-hidden rounded-2xl border border-hairline/60 bg-panel shadow-2xl">
        ...
        <div className="min-h-0 flex-1 space-y-5 overflow-y-auto p-5">
          ...
          <div className="rounded-xl border border-accent/20 bg-accent/5 px-3.5 py-3 text-[11.5px] leading-relaxed text-ink-secondary">Send the task in the request: <code className="text-ink">{`{"task":"Check the failed build"}`}</code>. The Clawd keeps its model, tools, permissions, and computer setup.</div>
          <details className="group rounded-xl border border-hairline/45 bg-inset/45 px-4 py-3" open={Boolean(webhook)}>
            ...
            <div className="mt-4 space-y-4">
              ...
              <div><div className="mb-2 text-[11.5px] font-medium text-ink-secondary">Run on</div><div className="grid grid-cols-2 gap-2"><button type="button" onClick={() => setRunOn("clawd")} className={cn("rounded-xl border p-3 text-left", runOn === "clawd" ? "border-accent/70 bg-accent/10" : "border-hairline/50 bg-panel hover:bg-raised/60")}><div className="flex items-center gap-2 text-[12.5px] font-medium text-ink"><Laptop size={14} />This computer</div></button><button type="button" disabled={!cloudReady && runOn !== "cloud"} onClick={() => setRunOn("cloud")} className={cn("rounded-xl border p-3 text-left disabled:cursor-not-allowed disabled:opacity-45", runOn === "cloud" ? "border-accent/70 bg-accent/10" : "border-hairline/50 bg-panel hover:bg-raised/60")}><div className="flex items-center gap-2 text-[12.5px] font-medium text-ink"><Cloud size={14} />Cloud VM</div></button></div></div>
...
/** A destination-first webhook view: choose a Clawd endpoint on the left, then
      ...
      <div className="mx-auto max-w-[1120px] space-y-4">
        <header className="flex flex-wrap items-start justify-between gap-3">
          <div>
            ...
            <p className="mt-1 text-[12px] text-ink-secondary">Send a task to a Clawd when another tool reports an event.</p>
          </div>
        ...
        {state.webhooks.length === 0 ? (
          <div className="border-t border-hairline/40 px-6 py-16 text-center">
            ...
            <p className="mx-auto mt-2 max-w-[420px] text-[12.5px] leading-relaxed text-ink-secondary">Choose a Clawd, copy one command, and every request becomes a new task in its chat.</p>
            ...
            {bots.length === 0 && <p className="mt-3 text-[12px] text-warning">Create a Clawd first, then come back here.</p>}
          </div>
        ) : selected && (
              ...
              <div className="flex flex-wrap items-start justify-between gap-3 px-5 pt-5 md:px-7 md:pt-6">
                <div className="flex min-w-0 items-center gap-3">
                  ...
                  <div className="min-w-0">
                    ...
                    <div className="mt-1 flex flex-wrap items-center gap-1.5 text-[10.5px] text-ink-secondary"><span>{selectedBot?.name ?? "Deleted Clawd"}</span><span>·</span><span>{selected.runOn === "cloud" ? "Cloud VM" : "This computer"}</span></div>
                  </div>
                </div>
                <div className="flex items-center gap-2">
              ...
              {tab === "setup" ? (
                <div className="px-5 py-6 md:px-7">
                  <div className="max-w-[720px]">
                    ...
                    <p className="mt-1 text-[11.5px] leading-relaxed text-ink-secondary">Copy this command into Terminal and press Return. It starts a real task in {selectedBot?.name ?? "this Clawd"}&apos;s chat; edit the task text for whatever you want done.</p>
                    {credential ? (
                    ...
                    <div className="mt-3 flex items-start gap-2 text-[10.5px] leading-relaxed text-ink-secondary"><Laptop size={12} className="mt-0.5 shrink-0" /><span>Local only for now. Keep Clawd Bot open while sending the request.</span></div>
