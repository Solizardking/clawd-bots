...
/** Clawd's behaviour vocabulary — CursorAvatar's 39 states, under the
 * app's historical names. */
export type ClawdState = CursorState;
export const CLAWD_STATES = CURSOR_STATES;
...
} satisfies Record<string, ClawdState[]>;

export const CLAWD_COLOR_NAMES = [
  "green",
  "blue",
  "red",
  "orange",
...
export type ClawdColor = (typeof CLAWD_COLOR_NAMES)[number];
...
export const CLAWD_COLORS = {
  ...
  teal: "#01A492",
  coral: "#E5634E",
} satisfies Record<ClawdColor, string>;

export const CLAWD_MOTIONS = [
  "arrive",
  "switch",
  "customize",
  "alert",
...
export type ClawdMotion = "none" | (typeof CLAWD_MOTIONS)[number];
...
const KNOWN_STATES = new Set<string>(CLAWD_STATES);

/** Resolves any stored value — current, legacy or junk — to a real state. */
export function normalizeState(value: string | null | undefined): ClawdState | null {
  if (!value) return null;
  if (KNOWN_STATES.has(value)) return value as ClawdState;
 ...
 */
export const PICKABLE_STATES: ClawdState[] = [
  "idle", // expression 0
  "happy", // 2
  "curious", // 3
  "drowsy", // 4
 ...
 */
export function stateForBot(bot: MascotBotProfile): ClawdState {
