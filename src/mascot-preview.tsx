...
interface ScenarioLabels extends Partial<Record<ClawdState, string>> {}
...
} satisfies Record<Exclude<ClawdMotion, "none">, string>;

const MOTION_COLORS: ClawdColor[] = [
...
function Tuner({
  ...
  state: ClawdState;
  setState: (s: ClawdState) => void;
  expression: number | undefined;
  setExpression: (e: number | undefined) => void;
  color: ClawdColor;
  setColor: (c: ClawdColor) => void;
  forward: boolean;
  setForward: (v: boolean) => void;
}) {
  const handle = useRef<ClawdAvatarHandle>(null);
          ...
          <div className="chips">
            {CLAWD_COLOR_NAMES.map((c) => (
              <button
                ...
                onClick={() => setColor(c)}
              >
                <span className="swatch" style={{ background: CLAWD_COLORS[c] }} />
                {c}
              </button>
            ))}
          </div>
...
function MotionCard({
  ...
  motion: Exclude<ClawdMotion, "none">;
  index: number;
  replayAll: number;
  color: ClawdColor;
}) {
...
function Preview() {
  ...
  const [color, setColor] = useState<ClawdColor>("green");
  ...
  return (
    <main className="preview-shell">
      ...
      <section className="motion-grid" aria-label="Mascot motion library">
        {CLAWD_MOTIONS.map((motion, index) => (
          <MotionCard
            key={motion}
            motion={motion}
            index={index}
      ...
      <section className="expression-library" aria-labelledby="expression-heading">
        <div className="section-heading">
          ...
          <p>Move your pointer over any Clawd to test the responsive eyes.</p>
        </div>
            ...
            {CLAWD_COLOR_NAMES.map((c) => (
              <div className="matrix-row" key={c}>
                <div className="row-label">
                  <span className="swatch" style={{ background: CLAWD_COLORS[c] }} />
                  <strong>{c}</strong>
                  <code>{CLAWD_COLORS[c]}</code>
                </div>
                {PICKABLE_STATES.map((s) => (
                  <div className="mascot-cell" key={`${c}-${s}`}>
                    <ClawdAvatar color={c} state={s} size={86} label={`${c} ${s} clawd`} />
                  </div>
                ))}
              </div>
            ))}
