...
type SkillRecordingPayload = {
  ...
  interface Window {
    ogb?: {
      ...
      /** Receives a GitHub package URL opened through clawdbot://install. */
      onPackageInstall?(cb: (url: string) => void): () => void;
      /** Updates the native Dock/taskbar unread indicator. */
      setUnreadCount?(count: number): void;
      /** Opens a live desktop as a sandboxed window owned by Clawd Bot. */
      desktopViewer?: {
      ...
      /** Asks where to save a bot-created file (inside ~/.clawdbot), copies
