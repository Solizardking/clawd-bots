...
- **Keep it green.** `npm run typecheck && npm run test` must pass. Server changes need tests (see below).
...
Requirements: **Node from `.node-version`**, **npm**, and for actually chatting with a bot, at least one agent CLI
...
```sh
git clone <your-repository-url> clawd-bot && cd clawd-bot
npm ci

npm run dev:server    # harness server → 127.0.0.1:8799
npm run dev           # app → http://127.0.0.1:5199
npm run dev:desktop   # Electron shell (macOS/Ubuntu; keep server + Vite running)

npm run typecheck     # app + server
npm run test          # vitest suite (server unit + driver contract + API smoke)
npm run test:watch    # same, in watch mode
npm run check:electron # syntax-check the plain JS Electron entrypoints

npm run package:mac   # DMG + ZIP; requires Swift/Xcode tools
npm run package:linux # Ubuntu x64 .deb + AppImage; no Swift required
```

`npm run dev:desktop` downloads and verifies the pinned Cloudflare Tunnel connector for the current
platform and architecture before Electron starts. Later launches re-verify and reuse the staged
binary. Packaging continues to use `npm run build:cloudflared`, which stages every architecture the
...
- stable `Clawd Bot-amd64.deb` and `Clawd Bot.AppImage` copies used by the latest-download links;
...
[`clawdbot-releases`](https://github.com/milind-soni/clawdbot-releases) repository. Then verify the checksum
...
| `dist-server/` | **Build output.** Never hand-edit, never include in PRs — it's regenerated by `npm run build:server` at release time. |

Data lives in `~/.clawdbot/` (bots, transcripts, per-thread NDJSON event logs, config with keys).

## Tests

The suite is colocated (`server/**/*.test.ts`) and runs with `npm run test`. Three layers:
...
- **Never touch the real `~/.clawdbot`.** The setup file points `HOME` at a temp dir; keep it
  that way.
...
API keys are write-only: they land in `~/.clawdbot/config.json` via `PUT /api/config` and the API
...
- [ ] `npm run check:electron` passes for desktop-shell changes
- [ ] Ubuntu packaging changes pass `npm run package:linux` and `node scripts/verify-linux-package.mjs`
