...
Add OpenCode Go as a first-class Clawd Bot engine by running the maintained
OpenCode CLI over its official Agent Client Protocol (ACP) interface and selecting
models from the `opencode-go/*` provider.

The first implementation should reuse Clawd Bot's ACP runtime rather than call
...
provide inference; Clawd Bot does not currently have a provider-independent
...
- OpenCode accepts an `OPENCODE_API_KEY` environment variable. Clawd Bot should
...
the process lifecycle Clawd Bot uses for Grok and Gemini.
...
- load that session on the next Clawd Bot turn;
...
- concurrent Clawd Bot threads without shared ACP or credential state.
...
- OpenCode Go is shown as unavailable until both its CLI and credential are
  present; Clawd Bot never selects it as a runnable default otherwise.
- The setup flow works without restarting Clawd Bot and never silently executes
  an installer.
...
6. Can the existing ACP usage metadata distinguish cumulative session totals from
   per-turn usage so Clawd Bot does not double-count it?
