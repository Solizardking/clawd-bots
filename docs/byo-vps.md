...
Clawd Bot can turn a Linux server you already own into a bot's computer. The agent process stays on your
...
only surface exposed: Clawd Bot never opens a public port on the VPS, never stores your SSH key or
...
Clawd Bot connects only through a named alias in your `~/.ssh/config` — you type the alias into
...
**Host key first, by hand.** Connect once manually before pointing Clawd Bot at the alias:
...
- **No public ports.** The managed container is created with no published ports, and Clawd Bot refuses to
...
- **Firewall the VPS to SSH only**, ideally from your IP. Nothing Clawd Bot does needs any other inbound
  port open, so anything else open is pure attack surface.
- **Nothing sensitive is stored.** The only thing Clawd Bot persists is the alias name itself
  (`~/.clawdbot/config.json`); keys, passphrases, and agent state stay with SSH. The alias is also kept
...
Each bot owns one container on the VPS, named `clawdbot-vps-<bot>-<hash>` — stable across restarts and
...
- **Remove** is yours, done by hand when a bot no longer needs the server:
  `docker -H ssh://my-vps rm -f <container>`. Clawd Bot never deletes a container on its own.
