# Computer use & browser use in Clawd Bot

Decision doc, 2026-08-12. How bots in Clawd Bot get local computer use and
...
supply-chain contract tracked in [#113](https://github.com/milind-soni/Clawd Bot/issues/113): pinned archive
...
receives the validated MCP proxy contract. Xorg is tracked in [#79](https://github.com/milind-soni/Clawd Bot/issues/79);
GNOME/Wayland additionally requires WinRects v8 plus the exact Cua health-report contract tracked in
[#109](https://github.com/milind-soni/Clawd Bot/issues/109).
...
1. **Spawn from the Electron main process, never from the server/gateway
   ...
   Spawned from Electron main → the grant is Clawd Bot's, users see ONE
   prompt named Clawd Bot, and the bundled driver inherits it. Spawned from
...
- Ship the binary at `Clawd Bot.app/Contents/Resources/cua-driver`,
