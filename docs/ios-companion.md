...
The iOS app is a thin, native client for the Clawd Bot instance running on
...
an Clawd Bot account of its own.
...
The Mac must be running Clawd Bot and must not be asleep. Desktop
...
The sidecar keeps its device registry in `~/.clawdbot/devices.json`. That is
...
Tailscale** in the desktop setup alternatives. Clawd Bot then places the
...
Tailscale is optional. The direct path does not use an Clawd Bot-operated
...
The Clawd Bot control plane stores account and installation metadata plus
...
The Mac must remain awake with Clawd Bot running for chats, approvals, and
...
An Clawd Bot account is not required for nearby, manual, or Tailscale
...
swift test
xcodegen generate
xcodebuild -project ClawdCompanion.xcodeproj \
  -scheme ClawdCompanion \
  -sdk iphonesimulator \
