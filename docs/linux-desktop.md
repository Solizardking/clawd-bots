...
Clawd Bot has an Ubuntu 24.04 LTS x86_64 desktop beta. The Electron package embeds the harness server, so
...
- The native Electron window and embedded Clawd Bot server on GNOME Xorg and GNOME Wayland.
...
progress in [issue #29](https://github.com/milind-soni/Clawd Bot/issues/29) and the safety hold in
[issue #345](https://github.com/milind-soni/Clawd Bot/issues/345). Bundled
CUA supply-chain work is tracked in [issue #113](https://github.com/milind-soni/Clawd Bot/issues/113). Xorg is tracked in
[issue #79](https://github.com/milind-soni/Clawd Bot/issues/79), and guarded GNOME/Wayland support in
[issue #109](https://github.com/milind-soni/Clawd Bot/issues/109).

## Download packages

Choose one Ubuntu 24.04 x86_64 package from the latest release:

- [Debian package (`Clawd Bot-amd64.deb`)](https://github.com/milind-soni/clawdbot-releases/releases/latest/download/Clawd Bot-amd64.deb) — recommended; APT installs its desktop dependencies.
- [Portable AppImage (`Clawd Bot.AppImage`)](https://github.com/milind-soni/clawdbot-releases/releases/latest/download/Clawd Bot.AppImage) — does not install system files.
- [SHA-256 checksums](https://github.com/milind-soni/clawdbot-releases/releases/latest/download/SHA256SUMS-ubuntu-x64.txt)

Versioned packages and previous releases remain available on the
[releases page](https://github.com/milind-soni/clawdbot-releases/releases).
...
```sh
git clone https://github.com/milind-soni/Clawd Bot.git
cd Clawd Bot
corepack enable
...
- `release/Clawd Bot-<version>-amd64.deb`
- `release/Clawd Bot-<version>-x86_64.AppImage`
...
```sh
sudo apt install ./Clawd Bot-amd64.deb
```

Then open **Clawd Bot** from the GNOME application launcher. To remove it:

```sh
sudo apt remove clawdbot
```

The portable AppImage does not install system files:

```sh
chmod +x release/Clawd Bot-*-x86_64.AppImage
./release/Clawd Bot-*-x86_64.AppImage
```

For a downloaded release AppImage, use `Clawd Bot.AppImage` in place of the versioned path above.

Application data remains local in `~/.clawdbot`. Electron browser data and window state use the normal XDG
configuration directory (`~/.config/clawdbot` unless the environment overrides it).
...
./release/linux-unpacked/clawdbot
```

## Agent CLI discovery

Applications launched from GNOME do not inherit the same interactive shell `PATH` as a terminal. Clawd Bot
...
```sh
OMB_EXTRA_PATH=/your/custom/bin ./release/Clawd Bot-*-x86_64.AppImage
```

Restart Clawd Bot after installing or signing in to a CLI.
...
24.04 GNOME Wayland session. Clawd Bot detects Wayland before XWayland when both `WAYLAND_DISPLAY` and
...
automatically. Clawd Bot does not capture screen audio, remember the selected monitor after restart, or
...
starts with `--no-overlay`, so Cua's decorative full-screen X11 cursor surface is never created. Clawd Bot also
...
The Xorg runtime uses private sockets, standard permission mode, per-action Clawd Bot approvals,
...
then runs the same smoke against `/opt/Clawd Bot/clawdbot`. These lanes prove the embedded server and UI are
...
Run the CLI directly in a terminal, finish its sign-in flow, then restart Clawd Bot. If it lives outside the
...
```sh
chmod +x Clawd Bot-*-x86_64.AppImage
file Clawd Bot-*-x86_64.AppImage
```
