...
//
// Clawd Bot owns only the sandbox boundary: image preparation, container
...
export const IMAGE_REPOSITORY = "localhost/clawdbot/cua-local-vm";
export const IMAGE_LAYER_VERSION = "4";
export const IMAGE_LAYER_LABEL = "com.clawdbot.image-layer";
export const IMAGE = `${IMAGE_REPOSITORY}:driver-${CUA_DRIVER_VERSION}-v${IMAGE_LAYER_VERSION}`;
export const CONTAINER = "clawdbot-computer";
export const MANAGED_LABEL = "com.clawdbot.local-vm";
export const DRIVER_LABEL = "com.clawdbot.cua-driver";
export const BASE_IMAGE_LABEL = "com.clawdbot.cua-base";
export const WORKSPACE_LABEL = "com.clawdbot.workspace";
export const TARGET_LABEL = "com.clawdbot.local-vm-target";
...
export const CUA_SOCKET = "/run/user/1000/clawdbot-cua.sock";
export const CUA_EXECUTABLE = "/usr/local/libexec/clawdbot/cua-driver";
...
RUN printf '%s\\n' \\
      ...
      > /usr/local/bin/prepare-clawdbot-workspace.sh \\
    && chmod 0755 /usr/local/bin/prepare-clawdbot-workspace.sh
RUN printf '%s\\n' \\
      '#!/bin/sh' \\
      '/usr/local/bin/prepare-clawdbot-workspace.sh' \\
      'attempt=0' \\
      ...
      > /usr/local/bin/start-clawdbot-cua-driver.sh \\
    && chmod 0755 /usr/local/bin/start-clawdbot-cua-driver.sh
RUN printf '%s\\n' \\
      '' \\
      '[program:clawdbot-cua-driver]' \\
      'command=/usr/local/bin/start-clawdbot-cua-driver.sh' \\
      'user=cua' \\
...
function statusProblem(status: ContainerComputerStatus): string | null {
  ...
  if (!status.managed) return "The existing container was not created by Clawd Bot; recreate it";
...
export async function containerComputerStatus(
  ...
  try {
    ...
    // The prepared Clawd Bot derivative has not been built yet.
  }
  ...
  if (canProbe) {
    try {
      ...
      const readinessShot = "/tmp/clawdbot-readiness.png";
      await runner(
        status.runtime,
        cuaExecArgs([
          "call",
...
async function prepareManagedImage(runtime: Runtime, runner: CommandRunner): Promise<void> {
  ...
  const context = await mkdtemp(join(tmpdir(), "clawdbot-cua-image-"));
  try {
...
export async function containerComputerScreenshot(
  ...
  try {
    const screenshot = "/tmp/clawdbot-preview.png";
    await runner(
      status.runtime,
      cuaExecArgs([
        "call",
...
/** Cloud boxes still use Clawd Bot's high-latency REST adapter. Local VMs
