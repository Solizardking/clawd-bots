    ...
    const sendTurn = async (turn: SendTurnInput) => {
      ...
      const launchAttempt = async (attempt: number): Promise<void> => {
        ...
        if (turn.integrations?.composio) {
          mountMcpServer(appServerArgs, env, "clawdbot_connectors", turn.integrations.composio);
        }
        ...
        if (turn.integrations?.phone) {
          ...
          const prefix = "mcp_servers.clawdbot_phone";
          appServerArgs.push(
        ...
        if (turn.integrations?.solana) {
          mountMcpServer(appServerArgs, env, "solana", turn.integrations.solana);
        }
      ...
      const settle = (ok: boolean, stopReason: string | null) => {
        ...
        state.settled = true;
        for (const finish of [...asks.values()]) finish("deny", "Clawd Bot: the turn ended", "system");
      ...
      try {
        await request("initialize", { clientInfo: { name: "clawdbot", version: "1" } });
