...
async function runAdb(args: string[], options: { binary?: boolean; timeoutMs?: number } = {}): Promise<Buffer> {
  ...
  if (!adb) throw new Error("Android platform tools are unavailable. Reopen Clawd Bot or install adb.");
...
async function readNodes(serial: string) {
  ...
  const remotePath = "/data/local/tmp/clawd-window.xml";
...
async function handle(message: Json) {
  ...
  if (method === "initialize") return ok(id, { protocolVersion: String(params.protocolVersion ?? "2024-11-05"), capabilities: { tools: {} }, serverInfo: { name: "clawdbot-phone", version: "1" } });
