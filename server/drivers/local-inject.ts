...
export function codexLocalProviderArgs(
  ...
  if (!host) return [];
  const envKey = `CLAWDBOT_LOCAL_${host.id.toUpperCase().replace(/[^A-Z0-9]+/g, "_")}_API_KEY`;
...
export async function mergeLocalInject(
  ...
  const probe = env.CLAWDBOT_PROBE_LOCAL_INJECT ?? process.env.CLAWDBOT_PROBE_LOCAL_INJECT;
