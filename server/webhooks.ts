...
const runOnSchema = z.preprocess((value) => value === "maus" ? "clawd" : value, z.enum(["clawd", "cloud"]));
...
function cleanInput(input: WebhookTriggerInput): CleanWebhookInput {
  ...
  const runOn = input.runOn ?? "clawd";
  if (!name) fail(400, "Give the webhook a name");
  if (!botId) fail(400, "Choose a Clawd");
  if (runOn !== "clawd" && runOn !== "cloud") fail(400, "Choose where this webhook runs");
...
export class WebhookManager {
  ...
  create(input: JsonValue): CreatedWebhook {
    ...
    if (this.options.botState(clean.botId) === "missing") fail(400, "That Clawd no longer exists");
    const now = this.now();
  ...
  disableForBot(botId: string): void {
    ...
    for (const trigger of this.webhooks) {
      ...
      trigger.updatedAt = this.now();
      this.options.cancelQueued?.(trigger.id, "The assigned Clawd was deleted");
      this.emit(trigger);
      changed = true;
    }
  ...
  test(id: string, payload: JsonValue = { event: "clawd.test", message: "Test webhook delivery" }): WebhookReceiveResult | null {
    const trigger = this.webhooks.find((candidate) => candidate.id === id);
    if (!trigger) return null;
    const eventName = trigger.eventTypes?.[0] ?? "clawd.test";
    return this.dispatch(trigger, {
      payload,
      contentType: "application/json",
      eventName,
      userAgent: "Clawd Bot webhook tester",
  ...
  private dispatch(trigger: StoredWebhookTrigger, event: WebhookEvent): WebhookReceiveResult {
    ...
    if (this.options.botState(trigger.botId) === "missing") fail(410, "The assigned Clawd no longer exists");
  ...
  private captureVerification(trigger: StoredWebhookTrigger, event: WebhookEvent): WebhookReceiveResult {
    ...
    this.appendAttempt(trigger, event, {
      ...
      statusCode: 202,
      deliveryId,
      reason: "Test event captured; enable the webhook to start Clawd tasks",
    });
    this.save();
    this.emit(trigger);
