...
export function createBotPackageExport(input: {
  ...
  const id = portableKey(input.name, "clawd-package", new Set());
  ...
  const definition: BotPackageDefinition = {
    ...
    release: "1.0.0",
    name: input.name,
    tagline: `A portable Clawd Bot setup with ${bots.length} ${bots.length === 1 ? "bot" : "bots"}.`,
    summary: "Exported from Clawd Bot. Review the roles, rooms, playbooks, connector requirements, and paused routines before sharing or publishing.",
    category: "Community",
    author: { name: input.authorName?.trim() || "Clawd Bot user" },
    license: "Unspecified",
