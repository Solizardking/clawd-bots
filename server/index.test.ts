...
beforeAll(async () => {
  ...
  writeFileSync(
    join(home, ".clawdbot", "groups.json"),
    JSON.stringify([
      {
        id: "test-dm",
  ...
  writeFileSync(
    join(home, ".clawdbot", "messages-test-stranded-room-thread.json"),
    JSON.stringify({
  ...
  writeFileSync(
    join(home, ".clawdbot", "messages-test-cancel-room-thread.json"),
    JSON.stringify({
...
describe("harness HTTP API", () => {
  ...
  it("serves packaged UI assets and preserves API 404s", async () => {
    ...
    expect(await root.text()).toContain("Packaged Clawd Bot");
    ...
    expect(await spa.text()).toContain("Packaged Clawd Bot");
  ...
  it("exports every visible bot and imports the team without creating a room", async () => {
    ...
    expect((await api("POST", "/api/teams/export", {})).body.team.name).toBe("My Clawd Team");
  ...
  it("validates and persists the global room turn timeout", async () => {
    ...
    const disk = JSON.parse(readFileSync(join(home, ".clawdbot", "config.json"), "utf8"));
  ...
  it.skipIf(process.platform === "win32")("stores the credentials file with owner-only permissions", () => {
    expect(statSync(join(home, ".clawdbot", "config.json")).mode & 0o777).toBe(0o600);
  });
  ...
  it("creates an independent webhook, accepts a delivery, deduplicates it, and rotates its secret", async () => {
    ...
    if (process.platform !== "win32") {
      expect(statSync(join(home, ".clawdbot", "webhooks.json")).mode & 0o777).toBe(0o600);
    }
  });
...
describe("bot memory API", () => {
  ...
  const workspaceOf = (botId: string) => join(home, ".clawdbot", "workspaces", botId);
  ...
  it("refuses every coat of path traversal without reading the target", async () => {
    ...
    try {
      ...
      writeFileSync(join(home, ".clawdbot", "secret.md"), "TOP-SECRET-MARKER sibling");
