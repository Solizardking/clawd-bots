...
// inside the VM and Clawd Bot never exposes another inbound port.
...
export const REMOTE_CUA_SESSION = "clawdbot";
export const REMOTE_CDP_HELPER = "/opt/ogb/clawdbot-cdp.mjs";
...
export function remoteComputerBootstrapCommand(botName: string): string {
  ...
  return [
    ...
    `tmux has-session -t work 2>/dev/null || tmux new-session -d -s work 'echo; echo "  ▦ ${safeName}'"'"'s computer — Clawd Bot"; echo; exec bash -i'`,
    "echo bootstrapped",
  ].join("\n");
}
