...
import { createSolanaRuntime } from "./solana/runtime.ts";
import { isSolanaFamilyTool } from "./solana/tools.ts";
...
const solanaRuntime = createSolanaRuntime({
  secrets: {
    ...
    phantomAppId: cfg.solana?.phantomAppId,
    phantomApiPrivateKey: cfg.solana?.phantomApiPrivateKey,
    jupiterApiKey: cfg.solana?.jupiterApiKey,
  },
  startRelay: process.env.VITEST !== "true",
});
...
const solanaProxyPath = SPAWNED_PROXIES.solana;
...
function solanaIntegration() {
  return {
    command: process.execPath,
    args: [solanaProxyPath],
    env: {
      ...AGENTS_NODE_FLAG,
      OMB_HARNESS_URL: `http://127.0.0.1:${PORT}`,
      OMB_COMMS_TOKEN: COMMS_TOKEN,
...
async function startTurn(
  ...
  const persona = [
    `You are ${bot.name}, a personal bot in Clawd Bot.`,
  ...
  void (async () => {
    try {
      ...
      if (instance.adapter.capabilities.agentsMcp === true || instance.adapter.capabilities.composioMcp === true) {
        integrations.solana = solanaIntegration();
      }
      ...
      // user's work. Its private Clawd workspace is app-owned and changes
      ...
      await instance.adapter.sendTurn({
        ...
        system:
          ...
          (integrations.composio
            ...
            : "") +
          (integrations.solana
            ? " You have Solana tools: solana_list_wallets, solana_wallet_assets, solana_asset_get, solana_assets_search; pump tape tools pump_recent_launches, pump_search_launches, pump_token_enrichment, pump_relay_status; and Jupiter tools jupiter_token_price, jupiter_buy_quote, jupiter_buy_token. Buys spend real SOL from a named local wallet and require confirm:true. Phantom/cloud wallets cannot sign."
            : "") +
...
try {
  ...
  console.log(`clawdbot webhook receiver on ${webhookIngress.baseUrl}`);
} catch (error) {
  webhookIngressError = error instanceof Error ? error.message : String(error);
  console.error(`clawdbot webhook receiver unavailable: ${webhookIngressError}`);
}
...
async function runGroupMemberTurn(
  ...
  const roster = group.memberIds
    ...
    .join(", ");
  const system = [
    `You are ${bot.name}, a bot in the room "${group.name}" in Clawd Bot.`,
    bot.title && `Role: ${bot.title}.`,
    bot.description && `About: ${bot.description}`,
    ...
    integrations.agents &&
      "If a supported API key is missing, use request_credential to show the secure in-app card. Never ask the user to paste credentials into chat.",
    integrations.solana &&
      "You have Solana, pump-tape, and Jupiter tools. Buys spend real SOL from a named local wallet and require confirm:true.",
  ]
    .filter(Boolean)
    .join("\n");
...
function dispatchConnectorResume(entry: { botId: string; threadId: string; resumeKey: string; labels: string[] }) {
  ...
  const prompt = `Clawd Bot connection update: the user securely connected ${names}. Continue the task that paused for this connection. Do not ask them to connect it again.`;
  if (owner.bot.busy) {
...
function dispatchSecretResume(entry: SecretResumeEntry) {
  ...
  const prompt =
    entry.outcome === "provided"
      ? `Clawd Bot credential update: the user securely provided ${entry.label}. Continue the task that paused for it. You do not receive the secret and must not ask them to paste it into chat.`
      : `Clawd Bot credential update: the user declined to provide ${entry.label}. Continue without it if possible, or briefly explain the limitation. Do not ask them to paste it into chat.`;
  if (owner.bot.busy) {
...
function configStatus() {
  return {
    ...
    solana: {
      heliusConfigured: Boolean(cfg.solana?.heliusApiKey),
      phantomConfigured: Boolean(
        cfg.solana?.phantomOrganizationId && cfg.solana?.phantomAppId && cfg.solana?.phantomApiPrivateKey,
      ),
      jupiterConfigured: Boolean(cfg.solana?.jupiterApiKey),
    },
  };
}
  ...
  try {
    ...
    if (path.startsWith("/api/internal/")) {
      ...
      if (method === "POST" && path === "/api/internal/ask-bot") {
        ...
        const prefixed = `[Message from @${currentFrom.name}, another bot in this Clawd Bot workspace. Reply to them.]\n\n${message}`;
      ...
      if (method === "POST" && path === "/api/internal/solana/tools") {
        const body = await readBody(req);
        const name = typeof body.name === "string" ? body.name : "";
        if (!isSolanaFamilyTool(name)) return json(res, 400, { error: `Unknown Solana family tool: ${name}` });
        try {
          const result = await solanaRuntime.executeTool(name, body.arguments ?? {});
    ...
    // relay never has to expose the rest of Clawd Bot's control surface.
    ...
    if (method === "GET" && path === "/api/health") {
      return json(res, 200, { app: "clawdbot", name: "Clawd Bot", pid: process.pid, static: Boolean(STATIC_DIR) });
    }
    ...
    if ((method === "PUT" || method === "PATCH") && path === "/api/config") {
      ...
      if (externalSecretStorage) {
        ...
        if (persisted.solana) {
          if (persisted.solana.heliusApiKey !== undefined) persisted.solana.heliusApiKey = "";
          if (persisted.solana.phantomOrganizationId !== undefined) persisted.solana.phantomOrganizationId = "";
          if (persisted.solana.phantomAppId !== undefined) persisted.solana.phantomAppId = "";
          if (persisted.solana.phantomApiPrivateKey !== undefined) persisted.solana.phantomApiPrivateKey = "";
          if (persisted.solana.jupiterApiKey !== undefined) persisted.solana.jupiterApiKey = "";
        }
        saveConfig(persisted);
        syncCredentialEnv(patch);
        Object.assign(cfg, loadConfig());
        solanaRuntime.applySecrets({
          heliusApiKey: cfg.solana?.heliusApiKey,
          phantomOrganizationId: cfg.solana?.phantomOrganizationId,
          phantomAppId: cfg.solana?.phantomAppId,
          phantomApiPrivateKey: cfg.solana?.phantomApiPrivateKey,
    ...
    if (method === "GET" && path === "/api/solana/wallets") {
      const [local, phantom] = await Promise.all([
        solanaRuntime.localWallets.listLocalWallets(),
        solanaRuntime.solana.listWallets(),
      ]);
      return json(res, 200, { local: local.wallets, phantom: phantom.wallets });
    }
    if (method === "POST" && path === "/api/solana/wallets") {
      const body = await readBody(req);
      try {
        const wallet = await solanaRuntime.localWallets.generateLocalWallet(body);
    ...
    if (method === "POST" && path === "/api/solana/wallets/reveal") {
      const body = await readBody(req);
      try {
        const secret = await solanaRuntime.localWallets.revealLocalWalletSecret(body);
        return json(res, 200, { secret });
      } catch (error) {
        return json(res, 404, { error: error instanceof Error ? error.message : String(error) });
      }
    }
    if (method === "POST" && path === "/api/solana/assets") {
      const body = await readBody(req);
      try {
        const ownerAddress = typeof body.ownerAddress === "string" ? body.ownerAddress : body.owner_address;
        const assets = await solanaRuntime.solana.getWalletAssets({
          ownerAddress,
    ...
    if (m && method === "POST") {
      ...
      if (bot.cloudBackend === "vps") {
        ...
        if (m[2] === "join") {
          if (req.headers["x-clawdbot-companion"] === "1") {
            return json(res, 409, {
...
server.listen(PORT, "127.0.0.1", () => {
  console.log(`clawdbot server on http://127.0.0.1:${PORT}`);
});
