// Config + data dirs. One file, ~/.clawdbot/config.json, env fallbacks:
...
const solanaConfigSchema = z.object({
  heliusApiKey: optionalText,
  phantomOrganizationId: optionalText,
  phantomAppId: optionalText,
  phantomApiPrivateKey: optionalText,
  jupiterApiKey: optionalText,
});
...
const appConfigSchema = z.object({
  ...
  composio: z.object({ hostedGateway: z.boolean().optional(), apiKey: optionalText, userId: optionalText, sessionId: optionalText }).optional(),
...
export interface AppConfig {
  ...
  composio?: { hostedGateway?: boolean; apiKey?: string; userId?: string; sessionId?: string };
  box?: { token?: string };
  ...
  /** Write-only Solana credentials. Values never leave GET /api/config. */
  solana?: {
    heliusApiKey?: string;
...
export const DATA_DIR = process.env.CLAWD_DATA_DIR ?? process.env.OMB_DATA_DIR ?? join(homedir(), ".clawdbot");
...
export function loadConfig(): AppConfig {
  ...
  cfg.solana = { ...cfg.solana };
  if (process.env.HELIUS_API_KEY !== undefined) cfg.solana.heliusApiKey = process.env.HELIUS_API_KEY;
  if (process.env.PHANTOM_ORGANIZATION_ID !== undefined) cfg.solana.phantomOrganizationId = process.env.PHANTOM_ORGANIZATION_ID;
  if (process.env.PHANTOM_APP_ID !== undefined) cfg.solana.phantomAppId = process.env.PHANTOM_APP_ID;
  if (process.env.PHANTOM_API_PRIVATE_KEY !== undefined) cfg.solana.phantomApiPrivateKey = process.env.PHANTOM_API_PRIVATE_KEY;
  if (process.env.JUPITER_API_KEY !== undefined) cfg.solana.jupiterApiKey = process.env.JUPITER_API_KEY;
  return cfg;
}
...
export function syncCredentialEnv(patch: Partial<AppConfig>): void {
  const secrets: Array<[value: string | undefined, name: string]> = [
    ...
    [patch.solana?.heliusApiKey, "HELIUS_API_KEY"],
    [patch.solana?.phantomOrganizationId, "PHANTOM_ORGANIZATION_ID"],
    [patch.solana?.phantomAppId, "PHANTOM_APP_ID"],
    [patch.solana?.phantomApiPrivateKey, "PHANTOM_API_PRIVATE_KEY"],
    [patch.solana?.jupiterApiKey, "JUPITER_API_KEY"],
  ];
...
/** Merge a partial config into ~/.clawdbot/config.json (secrets never
...
export function saveConfig(patch: Partial<AppConfig>): void {
  ...
  for (const key of ["xai", "openaiCompat", "composio", "box", "opencodeGo", "tts", "imageGen", "profile", "rooms", "localVm", "features", "solana"] as const) {
