...
const EXCLUDES = `# Clawd Bot checkpoint excludes — never snapshotted, never removed by restore
.git/
.svn/
.hg/
node_modules/
...
function gitEnv(shadow: string, cwd: string): NodeJS.ProcessEnv {
  ...
  env.GIT_AUTHOR_NAME = "Clawd Bot Checkpoint";
  env.GIT_AUTHOR_EMAIL = "checkpoint@clawdbot.local";
  env.GIT_COMMITTER_NAME = "Clawd Bot Checkpoint";
  env.GIT_COMMITTER_EMAIL = "checkpoint@clawdbot.local";
  return env;
}
