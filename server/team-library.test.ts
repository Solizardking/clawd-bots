  ...
  teams: [
    {
      ...
      manifest: "teams/engineering/team.clawdteam.json",
...
describe("team library", () => {
  it("validates catalog paths and adds the trusted repository URL", () => {
    ...
    expect(parsed.repositoryUrl).toBe("https://github.com/milind-soni/clawdbot-teams");
  ...
  it("loads only the manifest selected by the trusted catalog", async () => {
    const fetcher = vi.fn(async (url: string | URL | Request) => {
      ...
      if (target === `${TEAM_LIBRARY_RAW_ROOT}/teams/engineering/team.clawdteam.json`) return response(manifest);
      return response({}, 404);
    }) as unknown as typeof fetch;

    const loaded = await fetchLibraryTeam("engineering", fetcher);
    if (loaded.format !== "clawd.team") throw new Error("expected a legacy team");
  ...
  it("normalizes public GitHub repository, blob, and raw links", () => {
    expect(githubManifestUrls("https://github.com/acme/team")).toEqual([
      ...
      "https://raw.githubusercontent.com/acme/team/main/team.clawdteam.json",
      "https://raw.githubusercontent.com/acme/team/master/botmrr.md",
      "https://raw.githubusercontent.com/acme/team/master/team.md",
      "https://raw.githubusercontent.com/acme/team/master/team.clawdteam.json",
    ]);
    expect(githubManifestUrls("https://github.com/acme/team/blob/main/presets/seo.clawdteam.json")).toEqual([
      "https://raw.githubusercontent.com/acme/team/main/presets/seo.clawdteam.json",
    ]);
    expect(githubManifestUrls("https://raw.githubusercontent.com/acme/team/main/team.clawdteam.json")).toEqual([
      "https://raw.githubusercontent.com/acme/team/main/team.clawdteam.json",
    ]);
  ...
  it("falls back from main to master for a repository link", async () => {
    const fetcher = vi.fn(async (url: string | URL | Request) =>
      String(url).endsWith("team.clawdteam.json") && String(url).includes("/master/")
        ? response(manifest)
        : response({}, 404),
    ) as unknown as typeof fetch;
