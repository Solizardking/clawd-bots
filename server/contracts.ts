...
export interface SendTurnInput {
  ...
  integrations?: {
    ...
    /** Cloud computer, reached through Clawd Bot's REST-to-MCP adapter.
    ...
    dweb?: { url: string };
    /** In-process Solana / pump / Jupiter tools, forwarded through a stdio
     * MCP proxy so wallet secrets never enter the engine CLI child. */
    solana?: { command: string; args: string[]; env: Record<string, string> };
  };
  cwd?: string;
}
