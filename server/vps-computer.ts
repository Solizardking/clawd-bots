...
export const VPS_MANAGED_LABEL = "com.clawdbot.vps";
export const VPS_CONTAINER_LABEL = "com.clawdbot.container";
export const VPS_VIEWER_LABEL = "com.clawdbot.vps-viewer";
export const VPS_CONTAINER_PREFIX = "clawdbot-vps";
...
const SCREENSHOT_PATH = "/tmp/clawdbot-vps-preview.png";
...
function statusProblem(status: VpsComputerStatus): string | null {
  ...
  if (!status.image) return `Prepare the pinned Clawd Bot Cua image on the VPS (Driver ${CUA_DRIVER_VERSION})`;
  if (status.container === "missing") return "No Clawd Bot container exists for this bot on the VPS";
  if (!status.imageMatches) return "The VPS container uses an incompatible or untrusted Clawd Bot image";
  if (!status.managed) return "The VPS container name is occupied by a container Clawd Bot did not create";
  ...
  if (status.security === "unsafe") return "The VPS container is missing Clawd Bot safety limits";
  if (status.container === "stopped") return "The Clawd Bot VPS container is stopped";
  ...
  const operation = async () => {
    ...
    try {
      ...
      if (action === "provision") {
        ...
        // never docker-rm a container Clawd Bot did not create, even one
        // squatting on our name.
        if (before.container === "missing") return before;
        if (!before.managed) {
          throw Object.assign(
            new Error("The VPS container name is occupied by a container Clawd Bot did not create — remove it on the VPS yourself"),
            { status: 409 },
          );
        }
