...
async function officialBinary() {
  const cache = join(root, "node_modules", ".cache", "clawdbot", `cua-driver-${release.version}`);
  ...
  const response = await fetch(url, { headers: { "user-agent": "Clawd Bot-packager" } });
...
await build({
  stdin: {
    ...
    resolveDir: root,
    sourcefile: "clawdbot-cua-entry.mjs",
    loader: "js",
  },
  bundle: true,
  platform: "node",
  ...
  format: "esm",
  banner: {
    js: 'import { createRequire as __clawdbotCreateRequire } from "node:module"; const require = __clawdbotCreateRequire(import.meta.url);',
  },
  outfile: bundle,
  logLevel: "silent",
});
