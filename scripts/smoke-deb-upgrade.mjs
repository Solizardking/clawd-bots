...
try {
  const status = execFileSync("dpkg-query", ["-W", "-f=${db:Status-Abbrev}", "clawdbot"], {
    encoding: "utf8",
    stdio: ["ignore", "pipe", "ignore"],
  }).trim();
  if (status.startsWith("ii")) fail("refusing to replace a pre-existing Clawd Bot installation");
} catch (error) {
...
const legacyApp = path.join(legacyRoot, "opt", "Clawd Bot");
const legacyResources = path.join(legacyApp, "resources");
const legacyDeb = path.join(temporary, "clawdbot_0.1.7_amd64.deb");
...
try {
  ...
  fs.writeFileSync(
    ...
    [
      ...
      "Version: 0.1.7",
      "Architecture: amd64",
      "Maintainer: Clawd Bot CI <ci@clawdbot.invalid>",
      "Description: Legacy Clawd Bot directory-mode upgrade fixture",
      "",
    ].join("\n"),
    { mode: 0o644 },
  );
  ...
  for (const directory of ["/opt/Clawd Bot", "/opt/Clawd Bot/resources"]) {
  ...
  for (const directory of [
    "/opt/Clawd Bot",
    "/opt/Clawd Bot/resources",
    "/opt/Clawd Bot/resources/cua-linux-x64",
  ]) {
  ...
  for (const executable of ["cua-driver", "cua-cursor-theme"]) {
    const file = path.join("/opt/Clawd Bot/resources/cua-linux-x64", executable);
  ...
  const chromiumSandbox = "/opt/Clawd Bot/chrome-sandbox";
  ...
  const installedVersion = execFileSync("dpkg-query", ["-W", "-f=${Version}", "clawdbot"], {
    encoding: "utf8",
  }).trim();
  console.log(
