...
function verifyCompliance(licenses, label) {
  ...
  for (const component of registry) {
    const packageId = component.properties?.find(
      (property) => property.name === "clawdbot:cargo:package-id",
    )?.value;
...
const executable = path.join(unpacked, "clawdbot");
...
try {
  ...
  const debAppRoot = path.join(extracted, "opt", "Clawd Bot");
  ...
  for (const expected of [
    "Name=Clawd Bot",
    "Exec=/opt/Clawd Bot/clawdbot %U",
    "Icon=clawdbot",
    "StartupWMClass=com.clawdbot.app",
    "Categories=Utility;",
  ]) {
