...
const executable = path.resolve(
  process.env.OMB_SMOKE_EXECUTABLE ?? path.join(root, "release", "linux-unpacked", "clawdbot"),
);
...
mkdirSync(path.join(home, ".clawdbot"), { recursive: true });
...
for (const appName of ["clawdbot", "Clawd Bot"]) {
...
const metadata = {
  ...
  pid: process.pid,
  embedded: true,
  host_bundle_id: "com.clawdbot.app",
};
...
try {
  ...
  if (health?.app !== "clawdbot" || health.static !== true) {
    throw new Error(`unexpected embedded health response: ${JSON.stringify(health)}`);
  }
  if (!String(title).includes("Clawd Bot")) throw new Error(`unexpected renderer title: ${title}`);
  ...
  if (sessionBlocked) {
    ...
    const activeUserData = ["clawdbot", "Clawd Bot"]
    ...
    const userData = ["clawdbot", "Clawd Bot"]
      .map((name) => path.join(xdgConfig, name))
      .find((directory) => existsSync(path.join(directory, "cua-connection.json")));
