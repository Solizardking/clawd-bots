...
1. Quit every Clawd Bot window.
2. Confirm no old Clawd Bot process remains.
3. Start the app once and wait for the local harness.
4. If it still fails, inspect the desktop log.

On macOS, packaged logs are under `~/Library/Logs/Clawd Bot`. On Windows they are under the Clawd Bot log directory in `%APPDATA%`.
...
Developers can set `OMB_DATA_DIR` to run against a clean Clawd Bot data directory. This is useful for reproduction and screenshots without touching a real workspace.
