...
# Clawd Bot 0.1.7 installed the application ancestors as 0775, which makes
...
if [ -n "${CLAWDBOT_POSTINSTALL_TEST_ROOT:-}" ]; then
  ...
  case "$TEST_ROOT" in
    ...
    *)
      echo "Clawd Bot test install root must stay under /tmp" >&2
      exit 1
      ;;
  esac
...
repair_directory() {
  ...
  if [ -L "$target" ] || [ ! -d "$target" ]; then
    echo "Clawd Bot package directory is missing or unsafe: $target" >&2
    exit 1
  fi
  ...
  if [ "$actual" != "$EXPECTED_OWNER:755" ]; then
    echo "Clawd Bot could not secure package directory: $target ($actual)" >&2
    exit 1
  fi
}
...
repair_executable() {
  ...
  if [ -L "$target" ] || [ ! -f "$target" ]; then
    echo "Clawd Bot package executable is missing or unsafe: $target" >&2
    exit 1
  fi
  if [ "$TEST_MODE" -eq 0 ]; then chown root:root -- "$target"; fi
  chmod 0755 -- "$target"
  actual="$(stat -c '%U:%G:%a' -- "$target")"
  if [ "$actual" != "$EXPECTED_OWNER:755" ]; then
    echo "Clawd Bot could not secure package executable: $target ($actual)" >&2
    exit 1
  fi
}
...
repair_chromium_sandbox() {
  target=$1
  if [ -L "$target" ] || [ ! -f "$target" ]; then
    echo "Clawd Bot Chromium sandbox is missing or unsafe: $target" >&2
    exit 1
  fi
  if [ "$TEST_MODE" -eq 0 ]; then chown root:root -- "$target"; fi
  chmod 4755 -- "$target"
  ...
  if [ "$actual" != "$EXPECTED_OWNER:4755" ]; then
    echo "Clawd Bot could not secure Chromium sandbox: $target ($actual)" >&2
    exit 1
  fi
}
