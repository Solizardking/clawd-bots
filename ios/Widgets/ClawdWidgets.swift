...
@main
struct ClawdWidgets: WidgetBundle {
    ...
    var body: some WidgetConfiguration {
        ActivityConfiguration(for: BotActivityAttributes.self) { context in
            ...
            DynamicIsland {
                ...
                ClawdFaceStill(color: context.attributes.color, state: ClawdState(rawValue: context.state.face) ?? .idle, size: 24)
            } compactTrailing: {
                compactTrailing(context)
            } minimal: {
                ClawdFaceStill(color: context.attributes.color, state: ClawdState(rawValue: context.state.face) ?? .idle, size: 22)
            }
            .keylineTint(ClawdPalette.color(context.attributes.color))
        }
    }
    ...
    private func compactTrailing(_ context: ActivityViewContext<BotActivityAttributes>) -> some View {
        ...
        case "needsYou":
            Image(systemName: "hand.raised.fill")
                ...
                .foregroundStyle(ClawdPalette.color(context.attributes.color))
        case "working":
            Image(systemName: "circle.dotted")
                .font(.system(size: 13, weight: .semibold))
                .foregroundStyle(.white.opacity(0.8))
        default:
            Circle().fill(ClawdPalette.color(context.attributes.color)).frame(width: 8, height: 8)
        }
    }
}
                        ...
                        .background(
                            Capsule().fill(
                                option.caseInsensitiveCompare("Deny") == .orderedSame
                                    ...
                                    : ClawdPalette.color(context.attributes.color)
                            )
                        )
                }
                .buttonStyle(.plain)
...
private struct OrbitingFace: View {
    ...
    var body: some View {
        ZStack {
            ...
            ClawdFaceStill(color: context.attributes.color, state: ClawdState(rawValue: context.state.face) ?? .idle, size: size, comets: true, at: Date())
        }
    }
}
