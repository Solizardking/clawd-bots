                ...
                if let shown {
                    VStack(spacing: 10) {
                        ...
                        Button { open(shown.chat) } label: {
                            ChatAvatarView(chat: shown.chat, size: 120, state: ClawdState.forChat(shown.chat, in: session.state), animated: attentionLive, comets: attentionLive)
                        }
                        .buttonStyle(.plain)
                        ...
                        VStack(spacing: 4) {
                            Label("\(shown.chat.name) needs you", systemImage: "hand.raised.fill")
                                ...
                                .foregroundStyle(ClawdPalette.color(shown.chat.color))
                                ...
                                ForEach(card.options, id: \.self) { option in
                                    Button {
                                        ...
                                        Text(option)
                                            ...
                                            .frame(height: 40)
                                            .background(
                                                Capsule().fill(CardStyle.isRefusal(option) ? Color.white.opacity(0.16) : ClawdPalette.color(shown.chat.color))
                                            )
                                    }
                                    .buttonStyle(.plain)
                                    .disabled(answering)
