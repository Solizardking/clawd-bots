...
private struct RoutineEditorView: View {
    ...
    init(routine: Routine?, onSaved: @escaping () async -> Void) {
        ...
        _runOn = State(initialValue: routine?.runLocation ?? .clawd)
            ...
            Form {
                ...
                Section {
                    ...
                    if runOn == .clawd {
                        ...
                        Text("Runs the agent and its tools inside its Box virtual machine. The VM wakes automatically for each run; keep Clawd Bot running so its scheduler can launch the job.")
                    } else {
                ...
                Section {
                    ...
                    if kind == .once {
                        ...
                        Label(
                            "This routine uses a schedule added by a newer Clawd Bot. Choose One time or Selected days before saving.",
