...
enum ClawdPalette {
    /// src/lib/mascot.ts — CLAWD_COLORS
...
enum ClawdSilhouette {
    ...
    static func fit(_ rect: CGRect) -> CGAffineTransform {
        let side = ClawdFaceData.faceBox + margin * 2
...
/// on the desktop. `ClawdFaceEngine` is a port of the frame loop in
/// `ClawdMascotios/ClawdAvatar.tsx`: a state picks a pool of expressions and
...
struct ClawdAvatar: View {
    ...
    @State private var engine = ClawdFaceEngine()
...
final class ClawdFaceEngine {
    private(set) var state: ClawdState = .idle
    private var expression = 0
    private var currentRings: [[CGPoint]] = [ClawdFaceData.ring(0, eye: 0), ClawdFaceData.ring(0, eye: 1)]
    private var targetRings: [[CGPoint]] = [ClawdFaceData.ring(0, eye: 0), ClawdFaceData.ring(0, eye: 1)]
    private var currentMouth = ClawdFaceData.mouth(0)
    private var targetMouth = ClawdFaceData.mouth(0)
    private var currentGaze = ClawdFaceData.gaze(0)
    private var targetGaze = ClawdFaceData.gaze(0)
    ...
    func setState(_ new: ClawdState, now: Date) {
        guard !started || new != state else { return }
        started = true
        state = new
        stateStart = now
        select(ClawdFaceData.pools[new]?.first ?? 0)
        if last == nil { morph = 1; velocity = 0 } // first frame: rest on it, no morph in
        nextExpressionAt = schedule(ClawdFaceData.expressionCadence[new], from: now)
        nextBlinkAt = schedule(ClawdFaceData.blink[new], from: now)
    }
    ...
    private func select(_ index: Int) {
        let i = ((index % ClawdFaceData.expressionCount) + ClawdFaceData.expressionCount) % ClawdFaceData.expressionCount
        if i == expression && morph >= 1 { return }
        currentRings = displayedRings()
        currentMouth = displayedMouth()
        currentGaze = displayedGaze()
        targetRings = [ClawdFaceData.ring(i, eye: 0), ClawdFaceData.ring(i, eye: 1)]
        targetMouth = ClawdFaceData.mouth(i)
        targetGaze = ClawdFaceData.gaze(i)
        expression = i
        morph = 0
        velocity = 0
    }
    ...
    func step(now: Date) {
        ...
        if let due = nextExpressionAt, now >= due {
            let pool = ClawdFaceData.pools[state] ?? [0]
            let alternatives = pool.filter { $0 != expression }
            select(alternatives.randomElement() ?? pool[0])
            nextExpressionAt = schedule(ClawdFaceData.expressionCadence[state], from: now)
        }
        if let due = nextBlinkAt, now >= due {
            blinkStart = now
            nextBlinkAt = schedule(ClawdFaceData.blink[state], from: now)
        }
    }
    ...
    static let cometTrails: [ClawdState: CometSpec] = [
    ...
    func draw(in context: inout GraphicsContext, size: CGSize, color: String, bodyMotion: Bool, comets: Bool = false, at now: Date = Date()) {
        ...
        context.concatenate(ClawdSilhouette.fit(rect))
        ...
        let centre = CGPoint(x: ClawdFaceData.faceBox / 2, y: ClawdFaceData.faceBox / 2)
        ...
        if bodyMotion {
            bodyContext.concatenate(bodyTransform(ClawdFaceData.motion[state] ?? ClawdBodyMotion(), elapsed: elapsed))
        }
    ...
    private func drawBody(in context: inout GraphicsContext, color: String, now: Date) {
        let body = ClawdSilhouette.inFaceBox
        let bounds = ClawdSilhouette.faceBoxBounds
        context.fill(body, with: .linearGradient(
            Gradient(stops: ClawdPalette.gradientStops(color)),
        ...
        let a = ClawdFaceData.anchor
        context.translateBy(x: a.x, y: a.y)
        context.scaleBy(x: a.scale, y: a.scale)
        context.translateBy(x: -ClawdFaceData.faceCentre.x, y: -ClawdFaceData.faceCentre.y)
        ...
        context.stroke(
            ...
            with: .color(.white),
            style: StrokeStyle(lineWidth: ClawdFaceData.mouthStroke, lineCap: .round)
        )
    }

    /// The desktop's `bodyTransform`, in face-box units, elapsed in ms.
    private func bodyTransform(_ m: ClawdBodyMotion, elapsed: CGFloat) -> CGAffineTransform {
        let centre = ClawdFaceData.faceBox / 2
        let ground = ClawdFaceData.faceBox
