            ...
            .fullScreenCover(isPresented: $showingScanner) {
                PairingScannerSheet { payload in
                    guard let url = URL(string: payload), let invite = PairingInvite.parse(url) else {
                        return "That isn't an Clawd Bot pairing QR code."
                    }
                    accept(invite)
                    return nil
                }
    ...
    private var pairingHero: some View {
        VStack(spacing: 16) {
            ...
            VStack(spacing: 8) {
                ...
                Text("Scan the QR code in Clawd Bot. We'll securely choose the best way to connect.")
                    .font(.body)
        ...
        VStack(spacing: 22) {
            ...
            if let credential = scannedCredential {
                if !connectionIsProtected(connection) {
                    Text("Only continue on a network you trust. Local connections are authenticated but are not encrypted by Clawd Bot.")
                        .font(.footnote)
                        .foregroundStyle(.secondary)
                        .multilineTextAlignment(.center)
                        .fixedSize(horizontal: false, vertical: true)
