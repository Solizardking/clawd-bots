                        ...
                        ForEach(Array(rows.enumerated()), id: \.element.id) { index, summary in
                            NavigationLink(value: summary.chat) {
                                ChatRow(
                                    ...
                                    state: ClawdState.forChat(summary.chat, in: session.state),
        ...
        HStack(alignment: .top, spacing: 0) {
            ...
            ZStack {
                if chat.unread && !chat.busy {
                    Circle()
                        .fill(ClawdPalette.color(chat.color))
                        .frame(width: 10, height: 10)
                }
            }
            .frame(width: 22)
                ...
                VStack(alignment: .leading, spacing: 4) {
                    ...
                    if waiting {
                        Label("Waiting on you", systemImage: "hand.raised.fill")
                            ...
                            .padding(.vertical, 4)
                            .background(Capsule().fill(ClawdPalette.color(chat.color)))
                            .padding(.top, 4)
                    }
                }
                .padding(.vertical, 12)
                        ...
                        if let first = updates.first {
                            ...
                            case .needsYou:
                                Image(systemName: "hand.raised.fill")
                                    ...
                                    .foregroundStyle(ClawdPalette.color(first.chat.color))
    ...
    var body: some View {
        HStack(spacing: -overlap) {
            ForEach(Array(colors.enumerated()), id: \.offset) { _, color in
                ClawdAvatar(color: color, size: size, state: .idle, animated: false)
                    .padding(2)
                    .background(Circle().fill(Color(uiColor: .systemBackground)))
            }
        }
