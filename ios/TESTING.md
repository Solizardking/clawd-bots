...
```sh
git clone https://github.com/milind-soni/Clawd Bot
cd Clawd Bot
...
dns-sd -B _clawdbot._tcp                         # macOS: should list the service
```
...
- **The firewall is prompting.** System Settings → Network → Firewall. Incoming
  connections to `node`/Clawd Bot must be allowed, or the phone reaches
...
cd ios && xcodegen generate && open ClawdCompanion.xcodeproj
```
...
1. **Pair.** In Clawd Bot → Settings → Companion, choose **Set up a
   phone**. Scan the QR code with the phone's Camera, open ClawdMobile,
   ...
   **Connect**. The computer should also appear by name for the manual path:
   ...
   - If the list stays empty, check in this order:
     1. **Local Network permission.** iOS asks once, and a denial is
        permanent and silent. Settings → Clawd Bot → Local Network. If the
...
3. **In Clawd Bot → Settings → Phone:** with Phone access on, the panel now
...
- **Xcode console**, subsystem `com.clawdbot.companion`: `opening stream`,
