...
public struct PairingInvite: Equatable, Sendable {
    ...
    public static func parse(_ url: URL) -> PairingInvite? {
        guard url.scheme?.lowercased() == "clawdbot",
...
public struct PairingRouteError: Error, LocalizedError, Equatable, Sendable {
    ...
    public var errorDescription: String? {
        ...
        return "Couldn’t reach this computer through any available route (\(routes)). Keep Phone access turned on in Clawd Bot, then try again."
    }
}
...
public struct CompanionClient: Sendable {
    ...
    /// Only the first response that identifies itself as Clawd Bot receives
    ...
    private static func healthy(_ connection: Connection, session: URLSession) async -> Bool {
        do {
            ...
            guard !Task.isCancelled,
                  ...
                  try JSONDecoder().decode(HealthIdentity.self, from: data).app == "clawdbot"
            else { return false }
            return true
        } catch {
            return false
